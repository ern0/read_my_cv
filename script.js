...
function render_sidepanel() {
	...
	var content = render_sidepanel_copy_button();
	content += render_sidepanel_header();
...
function render_sidepanel_copy_button() {

	var content = "<div"
	content += " class=" + quote("copy_div")
	content += ">"

	content += "<input"
	content += " id=" + quote("copy")
	content += " type=" + quote("button")
	content += " value=" + quote("copy")
	content += " class=" + quote("copy_button")
	content += " onclick=" + quote("copy_url_clicked()");
    content += " />"
...
function copy_url_clicked() {

	var elm = document.getElementById("copy");
	if (elm == null) return;

	text = app.url.split("?")[1];
	text = "./launch.sh " + quote("?" + text);
	navigator.clipboard.writeText(text);

	elm.classList.add("copy_flash");
	setTimeout(function() {
		elm.classList.remove("copy_flash");
	},320);
}
